...
from models.gtrmma import DAPlanner, GTrajRecData, GTrajRecTestData, GTrajRecovery
...
def collate_fn(data0):
    ...
    trg_rid_labels, d_rids, d_rates, adj_matrices = zip(*data)
    ...
    # 邻接矩阵保持为列表（每个样本的图结构可能不同）
    adj_matrices = list(adj_matrices)

    return src_seqs, src_pro_feas, src_seg_seqs, src_seg_feats, src_lengths, trg_rids, trg_rates, trg_lengths, trg_rid_labels, da_routes, da_lengths, da_pos, d_rids, d_rates, adj_matrices
...
def collate_fn_test(data):
    ...
    return src_seqs, src_pro_feas, src_seg_seqs, src_seg_feats, src_lengths, trg_gps_seqs, trg_rids, trg_rates, trg_lengths, trg_rid_labels, da_routes, da_lengths, da_pos, d_rids, d_rates, adj_matrices
...
def train(model, iterator, optimizer, rid_features_dict, parameters, device):
    ...
    for i, batch in enumerate(iterator):
        t1 = time.time()
        src_seqs, src_pro_feas, src_seg_seqs, src_seg_feats, src_lengths, trg_rids, trg_rates, trg_lengths, trg_rid_labels, da_routes, da_lengths, da_pos, d_rids, d_rates, adj_matrices = batch
        ...
        output_ids, output_rates = model(src_seqs, src_lengths, trg_rids, trg_rates, trg_lengths,
                                        ...
                                        teacher_forcing_ratio=parameters.tf_ratio)
    ...
    with torch.no_grad():
        for i, batch in enumerate(iterator):
            ...
            output_ids, output_rates = model(src_seqs, src_lengths, trg_rids, trg_rates, trg_lengths,
                                            ...
                                            da_routes, da_lengths, da_pos, src_seg_seqs, src_seg_feats, 
                                            adj_matrices, d_rids, d_rates,
...
def infer(model, iterator, rid_features_dict, device):
    ...
    with torch.no_grad():
        for i, batch in enumerate(iterator):
            src_seqs, src_pro_feas, src_seg_seqs, src_seg_feats, src_lengths, trg_gps_seqs, trg_rids, trg_rates, trg_lengths, trg_rid_labels, da_routes, da_lengths, da_pos, d_rids, d_rates, adj_matrices = batch
...
def main():
    ...
    if opts.model_old_path != '':
        ...
        model_save_path = model_save_root + 'GTR_' + opts.city + '_' + 'keep-ratio_' + str(opts.keep_ratio) + '_' + time.strftime("%Y%m%d_%H%M%S") + '/'
    ...
    if opts.train_flag:
        # load dataset
        train_dataset = GTrajRecData(rn, traj_root, mbr, args, 'train')
        valid_dataset = GTrajRecData(rn, traj_root, mbr, args, 'valid')
        ...
        model = GTrajRecovery(args).to(device)
        ...
        best_valid_loss = float('inf')  # 使用RID loss作为主要指标
        best_epoch = 0
