...
from models.testDemo1 import TestDemo1
from models.trmma import DAPlanner, TrajRecData, TrajRecTestData
...
def train(model, iterator, optimizer, rid_features_dict, parameters, device):
    ...
    for i, batch in enumerate(iterator):
        ...
        output_ids, output_rates = model(src_seqs, src_lengths, trg_rids, trg_rates, trg_lengths,
                                        src_pro_feas, rid_features_dict, da_routes, da_lengths, da_pos, src_seg_seqs, src_seg_feats, d_rids, d_rates, teacher_forcing_ratio=parameters.tf_ratio, mode='train')
    ...
    with torch.no_grad():
        for i, batch in enumerate(iterator):
            ...
            output_ids, output_rates = model(src_seqs, src_lengths, trg_rids, trg_rates, trg_lengths,
                                            ...
                                            teacher_forcing_ratio=0, mode='iterative')
    ...
    with torch.no_grad():
        for i, batch in enumerate(iterator):
            ...
            output_ids, output_rates = model(src_seqs, src_lengths, trg_rids, trg_rates, trg_lengths,
                                            src_pro_feas, rid_features_dict,
                                            da_routes, da_lengths, da_pos, src_seg_seqs, src_seg_feats, d_rids, d_rates,
                                            teacher_forcing_ratio=-1, mode='iterative')
...
def main():
    parser = argparse.ArgumentParser(description='TestDemo1')
    ...
    parser.add_argument('--max_iterations', type=int, default=3, help='maximum iterations for iterative optimization')
    ...
    if opts.model_old_path != '':
        ...
        model_save_root = f'./model/TestDemo1/{opts.city}/'
        model_save_path = model_save_root + 'TestDemo1_' + opts.city + '_' + 'keep-ratio_' + str(opts.keep_ratio) + '_' + time.strftime("%Y%m%d_%H%M%S") + '/'
    ...
    args_dict = {
        ...
        'utc': utc,
        'small': opts.small,
        'dam_root': os.path.join("data", opts.city) if opts.city != "porto" else os.path.join("data", "porto_large"),
